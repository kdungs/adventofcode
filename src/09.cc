...
auto shortest_roundtrip(const DistanceMatrix& dists) {
  ...
  do {
    ...
    auto dist = std::accumulate(std::begin(distances) + 1, std::end(distances), 0u);
    if (dist < minDist) {
      minDist = dist;
    }
...
int main(int argc, char* argv[]) {
  ...
  auto distances = DistanceMatrix{dists};
  std::cout << shortest_roundtrip(distances) << '\n';
}
